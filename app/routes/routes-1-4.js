module.exports = function (router) {
    ...
    router.get('/1-4/AO/action-reload-data', function (req, res) {
    ...
    router.post('/1-4/AO/ao-my-services', function (req, res) {
    ...
    router.get('/1-4/AO/action-ao-verify-tLevels', function (req, res) {
        ...
        res.redirect('/1-4/AO/ao-verify-tLevels?tl=' + req.session.data['requested_tLevel'])
    })

    router.get('/1-4/AO/action-verify-single-tLevel', function (req, res) {
        ...
        if (acceptance === 'no') {
            ...
            res.redirect('/1-4/AO/ao-inform-ifate')
        } else if (acceptance === 'yes') {
            // Accepted
            req.session.data['tLevels-ao'][idx][6] = "Verified"
            res.redirect('/1-4/AO/ao-t-levels')
        } else {
            res.redirect('/1-4/AO/ao-verify-tLevels')
        }
    })

    router.get('/1-4/AO/action-ao-providers', function (req, res) {
        ...
        res.redirect('/1-4/AO/ao-providers')
    })

    router.get('/1-4/AO/action-search-providers', function (req, res) {
    ...
    router.get('/1-4/AO/action-ao-views-provider', function (req, res) {
        /*
        ...
        */
        ...
        req.session.save()
        res.redirect('/1-4/AO/ao-views-provider')
    })

    router.post('/1-4/AO/action-import-providers-single', function (req, res) {
        /*
    ...
    router.post('/1-4/AO/action-edit-providers-single', function (req, res) {
        /* 
    ...
    router.post('/1-4/AO/action-ao-add-providers-bulk', function (req, res) {
        /*
    ...
    router.get('/1-4/AO/ao-many-providers-add-01', function (req, res) {
    ...
    router.get('/1-4/AO/action-add-centre-single-02', function (req, res) {
        ...
        res.redirect('/1-4/AO/ao-new-provider-add-03')
    })

    router.post('/1-4/AO/action-add-centre-single-03', function (req, res) {
        // Gather selected T Levels from checklist into a list
        res.redirect('/1-4/AO/ao-new-provider-add-04')
    })

    router.post('/1-4/AO/action-add-centre-single-04', function (req, res) {

        // Create confirmation message
        if (req.session.data['select-tLevel'].length > 1) {
            // Nicely formatted string: "a, b, c and d"
            tls = req.session.data['select-tLevel'].slice(0, -1).join(', ') + ' and ' + req.session.data['select-tLevel'].slice(-1)
            conf = "We have linked " + req.session.data['add-ao-selected'][1] + " with the T Levels " + tls
        } else {
            conf = "We have linked " + req.session.data['add-ao-selected'][1] + " with the T Level " + req.session.data['select-tLevel']
        }

        // Reset session variables
        req.session.data['errors'] = null
        req.session.data['add-ao-selected'] = null
        req.session.data['search-matches'] = null
        req.session.data['centre-question'] = null
        req.session.data['select-tLevel'] = null
    ...
    router.post('/1-4/AO/action-add-centre-many-01', function (req, res) {
        ...
        if (search.length < 1) {
            ...
            // Render page
            res.redirect('/1-4/AO/ao-many-providers-add-02')
        }
    })

    router.get('/1-4/AO/action-add-centre-many-02', function (req, res) {
        // Get index of selected provider
        var idx = req.query['index']
        req.session.data['add-ao-selected'] = req.session.data['search-matches'][idx]
    ...
    // ####################### VIEW/EDIT PROVIDER PROCESS #######################

    router.post('/1-4/AO/action-view-centre-01', function (req, res) {
        ...
        if (search.length < 1) {
            ...
            // Render page
            res.redirect('/1-4/AO/ao-view-provider-02')
        }
    })

    router.get('/1-4/AO/action-view-centre-02', function (req, res) {
        // Get index of selected provider
        var idx = req.query['index']
        req.session.data['add-ao-selected'] = req.session.data['search-matches'][idx]
        ...
        res.redirect('/1-4/AO/ao-view-provider-03')
    })

    // ####################### ADD STUDENTS PROCESSES #######################

    router.post('/1-4/AO/action-students-question-next', function (req, res) {
        req.session.data['errors'] = []
        ...
        if (answer === 'add-one') {
            ...
            req.session.data['errors'].push(error)
            res.redirect('/1-4/AO/ao-view-students')
        }
    })

    router.post('/1-4/AO/action-add-student-single-01', function (req, res) {
        /*
        User first selects the T Level the learner is doing
        */
        res.redirect('/1-4/AO/ao-add-student-single-02')
    })

    router.post('/1-4/AO/action-add-student-single-02', function (req, res) {
        /*
        ...
        */
        ...
        if (tlevel === undefined) {
            ...
            req.session.data['errors'].push(error)
            res.redirect('/1-4/AO/ao-add-student-single-01')
        } else {
            res.redirect('/1-4/AO/ao-add-student-single-02')
        }
    })

    router.post('/1-4/AO/action-add-student-single-03', function (req, res) {
        /*
        ...
        */
        ...
        if (year === undefined) {
            ...
            res.redirect('/1-4/AO/ao-add-student-single-03')
        }
    })

    router.post('/1-4/AO/action-add-student-single-04', function (req, res) {
        /*
        ...
        */
        ...
        if (uln.length < 10) {
            ...
            res.redirect('/1-4/AO/ao-add-student-single-04')
        }
    })

    router.post('/1-4/AO/action-add-student-single-check', function (req, res) {
        req.session.data['errors'] = []
        ...
        res.redirect('/1-4/AO/ao-add-student-single-check')
    })

    router.post('/1-4/AO/action-view-students-details', function (req, res) {
        ...
        if (uln.length < 10) {
            ...
            error = ['#01', 'The ULN you provided is not of a correct format']
            req.session.data['errors'].push(error)
            res.redirect('/1-4/AO//ao-view-student')
        } else {
            ...
            res.redirect('/1-4/AO/ao-views-student')
        }
    })

    router.post('/1-4/AO/action-ao-add-students-bulk', function (req, res) {
        /*
        Adds students in bulk
        */
        ...
        res.redirect('/1-4/AO/action-ao-view-students')
    })

    router.post('/1-4/AO/action-add-student-single-confirm', function (req, res) {
        /*
        ...
        */
        ...
        var line = [
            ...
            '']
        req.session.data['students-ao'].unshift(line)
        //res.redirect('/1-4/AO/ao-add-student-single-confirm')
    ...
    router.get('/1-4/AO/action-ao-view-students', function (req, res) {
        /*
        Pagination!
        */
        //checkIfActive(req)
    ...
    router.get('/1-4/AO/action-ao-views-student', function (req, res) {
        /*
    ...
    router.get('/1-4/AO/action-search-students', function (req, res) {
        /*
        ...
        */
        ...
        //req.session.data['students-ao-tmp'] = req.session.data['students-ao']
        res.redirect('/1-4/AO/ao-view-students?pageNumber=' + queryString)

    })

    router.get('/1-4/AO/action-clear-students-search', function (req, res) {
    ...
    router.get('/1-4/AO/action-clear-providers-search', function (req, res) {
    ...
    router.get('/1-4/AO/action-upload-assessments', function (req, res) {

        res.redirect('/1-4/AO/ao-add-assessments')
    })
}
